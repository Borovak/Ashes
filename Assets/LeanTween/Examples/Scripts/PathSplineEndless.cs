...
namespace LeanTween.Examples.Scripts
{
	public class PathSplineEndless : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.value(gameObject, 0, 0.3f, 2f).setOnUpdate( ( float val )=>{
				pushTrackAhead = val;
			});
		}
		...
		void Update () {
			...
			if(UnityEngine.Input.anyKeyDown){
				...
				Framework.LeanTween.moveLocalX(carInternal, (trackIter-1)*6f, 0.3f).setEase(LeanTweenType.easeOutBack);
			}
		}
		...
		void addRandomTrackPoint(){
			...
			Framework.LeanTween.rotateAround( tree, Vector3.forward, 0f, 1f).setFrom( zIter%2==0 ? 180f : -180f).setEase(LeanTweenType.easeOutBack);
