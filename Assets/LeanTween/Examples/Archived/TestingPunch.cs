...
namespace LeanTween.Examples.Archived
{
    public class TestingPunch : MonoBehaviour {
        ...
        void Update () 
        {        
            Framework.LeanTween.dtManual = Time.deltaTime;
            if (UnityEngine.Input.GetKeyDown(KeyCode.Q))
            {
                ...
                Framework.LeanTween.moveLocalX(gameObject, 5, 1).setOnComplete( () => {
                ...
                Framework.LeanTween.value(lt.gameObject, lt.intensity, 0.0f, 1.5f)
            ...
            if (UnityEngine.Input.GetKeyDown(KeyCode.S))
            {
                ...
                Framework.LeanTween.scale(this.gameObject, new Vector3(1.15f, 1.15f, 1.15f), 0.6f);

                Framework.LeanTween.rotateAround(this.gameObject, Vector3.forward, -360f, 0.3f).setOnComplete(() =>
                {
                    Framework.LeanTween.rotateAround(this.gameObject, Vector3.forward, -360f, 0.4f).setOnComplete(() =>
                    {
                        Framework.LeanTween.scale(this.gameObject, new Vector3(1f, 1f, 1f), 0.1f);

                        Framework.LeanTween.value(this.gameObject, (v) =>
                        {
            ...
            if (UnityEngine.Input.GetKeyDown(KeyCode.T))
            {
                ...
                descr = Framework.LeanTween.move(gameObject, pts, 15f).setOrientToPath(true).setDirection(1f).setOnComplete( ()=>{
                    Debug.Log("move path finished");
                });
            }

            if (UnityEngine.Input.GetKeyDown(KeyCode.Y)) // Reverse the move path
            {
            ...
            if (UnityEngine.Input.GetKeyDown(KeyCode.R))
            {
                ...
                Framework.LeanTween.rotateAroundLocal(this.gameObject, this.transform.forward, -80f, 5.0f).setPoint(new Vector3(1.25f, 0f, 0f));
                print("rotate punch!");
            }
            ...
            if (UnityEngine.Input.GetKeyDown(KeyCode.M))
            {
                ...
                Framework.LeanTween.moveX( this.gameObject, 1f, 1f).setOnComplete( destroyOnComp ).setOnCompleteParam( this.gameObject ).setOnComplete( ()=>{
            ...
            {
                Framework.LeanTween.color( this.gameObject, new Color(1f, 0f, 0f, 0.5f), 1f);

                Color to = new Color(Random.Range(0f,1f),0f,Random.Range(0f,1f),0.0f);
                GameObject l = GameObject.Find("LCharacter");
                Framework.LeanTween.color( l, to, 4.0f ).setLoopPingPong(1).setEase(LeanTweenType.easeOutBounce);
            }

            if (UnityEngine.Input.GetKeyDown(KeyCode.E))
            {
                Framework.LeanTween.delayedCall(gameObject,0.3f, delayedMethod).setRepeat(4).setOnCompleteOnRepeat(true).setOnCompleteParam( "hi" );
            }

            if (UnityEngine.Input.GetKeyDown(KeyCode.V))
            {
                Framework.LeanTween.value( gameObject, updateColor, new Color(1.0f,0.0f,0.0f,1.0f), Color.blue, 4.0f );//.setRepeat(2).setLoopPingPong().setEase(LeanTweenType.easeOutBounce);
            }

            if (UnityEngine.Input.GetKeyDown(KeyCode.P))
            {
                Framework.LeanTween.delayedCall(0.05f, enterMiniGameStart).setOnCompleteParam( new object[]{""+5} );
            }

            if(UnityEngine.Input.GetKeyDown(KeyCode.U)){
#if !UNITY_FLASH
                Framework.LeanTween.value(gameObject, (Vector2 val)=>{
                ...
                Framework.LeanTween.value(l, new Vector2( l.transform.position.x, l.transform.position.y), new Vector2( l.transform.position.x, l.transform.position.y+5), 1f ).setOnUpdate( 
                    (Vector2 val)=>{
                        Debug.Log("tweening vec2 val:"+val);
        ...
        static void tweenStatically( GameObject gameObject ){
            ...
            Framework.LeanTween.value(gameObject, (val)=>{
